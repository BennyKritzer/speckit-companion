...
- **Compact optional tabs**: optional workflow tabs now use a compact `OO` badge while they have not been run yet; once the optional step starts, completes, or gets skipped, the badge disappears and the tab returns to the normal step presentation.
- **Quiet, intentional footer**: the footer is the canonical action surface — `Regenerate` plus a forward button labelled with the next phase (`Plan` / `Tasks` / `Implement` / `Complete`). The navigation bar keeps only the Activity toggle. While a step generates, that footer button is disabled and reads `Generating <step>…` until the artifact actually lands on disk, so the footer never advances ahead of the work. `Archive` and `Mark Completed` appear only once the spec is closure-eligible (`ready-to-implement` and beyond). See [`docs/viewer-states.md`](./docs/viewer-states.md) for the full footer state matrix.
